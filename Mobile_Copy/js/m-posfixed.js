  ...
  $.extend($.fn, {
    posfixed: function (configSettings) {
      ...
      if ($(settings.rightName).length > 0) {
        ...
        var left =
          (document.body.clientWidth - _this.outerWidth()) / 2 +
          $(settings.pdBoxInclusion).outerWidth() +
          (_this.outerWidth() -
            $(settings.pdBoxInclusion).outerWidth() -
            $(obj).outerWidth())
      }
      window.onresize = function () {
        if ($(settings.rightName).length > 0) {
          ...
          if (
            scrollTop >= initPos &&
            scrollTop < pic_wrap + tabContent - $(obj).outerHeight() - 10
          ) {
            ...
            scrollTop >=
            pic_wrap + tabContent - $(obj).outerHeight() - 10
          ) {
            $(obj).css({
      ...
      $(window).scroll(function () {
        ...
        var scrollTop =
          document.documentElement.scrollTop || document.body.scrollTop
        if (
          $(settings.rightName).length > 0 &&
          $(settings.rightName).outerHeight() < document.body.clientHeight
        ) {
          if (
            scrollTop >= initPos &&
            scrollTop < pic_wrap + tabContent - $(obj).outerHeight() - 10
        ...
        if (
          scrollTop >= headerTop - $('.wsc-TopCon').outerHeight() &&
          scrollTop <=
            $(settings.scrollLI)
              .eq($(settings.scrollLI).length - 1)
              .offset().top +
              $(settings.scrollLI)
                .eq($(settings.scrollLI).length - 1)
                .outerHeight() -
        ...
        if ($(settings.scrollLI).length) {
          for (var i = 0; i < $(settings.padtabBox).find('li').length; i++) {
            if (i === 0) {
              if (
                scrollTop <=
                $(settings.scrollLI).eq(1).offset().top -
                  $(settings.padtabBox).outerHeight()
              ) {
                $(settings.padtabBox)
                  .find('li')
                  .eq(0)
              ...
              i > 0 &&
              i <= $(settings.padtabBox).find('li').length - 2
            ) {
              if (
                scrollTop >=
                  $(settings.scrollLI).eq(i).offset().top -
                    $(settings.padtabBox).outerHeight() -
                    2 -
                    ($(settings.Mheader).outerHeight() || 0) &&
                scrollTop <
                  $(settings.scrollLI)
                    .eq(i + 1)
                    .offset().top -
              ...
              if (
                scrollTop >=
                $(settings.scrollLI)
                  .eq($(settings.padtabBox).find('li').length - 1)
                  .offset().top -
                  $(settings.padtabBox).outerHeight() -
                  2 -
                  ($(settings.Mheader).outerHeight() || 0)
      ...
      $(settings.padtabBox).on('click', 'li', function (e) {
        ...
        if (index === 0) {
          $('html,body').animate(
            { scrollTop: ScrollDistance - $(settings.padtabBox).outerHeight() },
            200,
          ) //滚动到标题位置
        } else {
          $('html,body').animate(
